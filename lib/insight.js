...
function Insight(url, network) {
  ...
  if (Networks.get(url)) {
    ...
    if (network === Networks.livenet) {
      url = 'http://insight.dash.org';
    } else {
      url = "http://test-insight.dev.dash.org/insight-api-dash"; //'http://test-insight.dash.org'; !!! remember to replace this
    }
  }
...
/**
 * Retrieve List of MasterNodes
 * @param {AddressCallback} callback
 */
Insight.prototype.mnlist = function(callback) {
    $.checkArgument(_.isFunction(callback));
    this.requestGet('/status?q=getMNlist', function(err, res, body) {
        if (err || res.statusCode !== 200) {
            return callback(err || res);
        }
        var mnlist = JSON.parse(body);
